...
def bbox_iou(
    ...
    if CIoU or DIoU or GIoU or MPDIoU:
        ...
        if MPDIoU:
            ...
            return iou - (d1 + d2) / (cw.pow(2) + ch.pow(2) + eps)
...
def wasserstein_loss(
    ...
    """Calculate Normalized Wasserstein Distance (NWD) similarity between bounding boxes.

    The implementation follows the common tiny-object detection formulation that models each horizontal box as a
    2D Gaussian and converts the 2-Wasserstein distance into a normalized similarity score in [0, 1].

    Args:
        box1 (torch.Tensor): Predicted boxes with last dimension 4.
        box2 (torch.Tensor): Target boxes with last dimension 4.
        xywh (bool): If True, inputs are (x, y, w, h); otherwise inputs are (x1, y1, x2, y2).
        constant (float): Normalization constant used by NWD.
        eps (float): Small value for numerical stability.

    Returns:
        (torch.Tensor): NWD similarity, same broadcasted shape as IoU outputs.
    """
    if xywh:
    ...
    return torch.exp(-wasserstein_2.sqrt() / constant)
