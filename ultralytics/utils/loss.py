...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, box_loss: str = "ciou", nwd_constant: float = 12.8):
        """Initialize the BboxLoss module with regularization maximum and DFL settings."""
        super().__init__()
        if box_loss not in {"ciou", "mpdiou", "nwd"}:
            raise ValueError(f"Unsupported box_loss={box_loss}. Supported values are 'ciou', 'mpdiou', and 'nwd'.")
    ...
    def forward(
        ...
        if self.box_loss == "nwd":
            similarity = wasserstein_loss(
                pred_bboxes[fg_mask],
            ...
            similarity = bbox_iou(
                ...
                target_bboxes[fg_mask],
                xywh=False,
                CIoU=self.box_loss == "ciou",
                MPDIoU=self.box_loss == "mpdiou",
            )
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(
            m.reg_max,
            getattr(h, "box_loss", "ciou"),
            getattr(h, "nwd_constant", 12.8),
        ).to(device)
