...
def convert_visdrone_det_to_yolo(root_dir: str | Path):
    """Convert a local VisDrone2019-DET dataset from original annotations to YOLO label files.

    The converter keeps the original image folders in place and writes labels beside each split:
    `VisDrone2019-DET-train/labels`, `VisDrone2019-DET-val/labels`, and `VisDrone2019-DET-test-dev/labels`.

    Args:
        root_dir (str | Path): Root directory containing the original VisDrone2019-DET split folders.
    """
    ...
    for split, folder in splits.items():
        split_dir = root_dir / folder
        images_dir = split_dir / "images"
        annotations_dir = split_dir / "annotations"
        labels_dir = split_dir / "labels"
        if not images_dir.exists() or not annotations_dir.exists():
            raise FileNotFoundError(f"Missing VisDrone {split} images or annotations under {split_dir}")

        labels_dir.mkdir(parents=True, exist_ok=True)
        image_paths = sorted(images_dir.glob("*.jpg"))
        converted_instances = 0
        for image_path in TQDM(image_paths, desc=f"Converting {folder}"):
            ...
            if annotation_path.exists():
                for raw_line in annotation_path.read_text(encoding="utf-8").splitlines():
                    ...
                    cls = int(float(row[5]))
                    if score == 0 or cls <= 0 or cls > class_count or w <= 0 or h <= 0:
                        continue

                    x1 = max(x, 0.0)
                    y1 = max(y, 0.0)
                    x2 = min(x + w, float(image_width))
                    y2 = min(y + h, float(image_height))
                    bw = x2 - x1
                    bh = y2 - y1
                    if bw <= 0 or bh <= 0:
                        continue

                    x_center = (x1 + bw / 2) / image_width
                    y_center = (y1 + bh / 2) / image_height
                    lines.append(
                        f"{cls - 1} {x_center:.6f} {y_center:.6f} {bw / image_width:.6f} {bh / image_height:.6f}\n"
                    )
        ...
        LOGGER.info(
            f"Converted {len(image_paths)} VisDrone {split} images and {converted_instances} instances to {labels_dir}"
        )
